package top.zhongnan.minidb.client.ui;

/**
 * REPL 提示符状态机。
 *
 *   IDLE         "minidb> "         灰色，普通待输入
 *   IN_TX        "miniDB* "         黄色 *，表示当前在事务里
 *   AFTER_ERROR  "miniDB! "         红色 !，上一条出错；下一次成功后自动回到 IDLE
...
public final class Prompter {
    ...
    public String mainPrompt() {
        switch (state) {
            case IN_TX:       return Theme.warn("miniDB*") + " " + Theme.muted(Theme.arrow() + " ");
            case AFTER_ERROR: return Theme.danger("miniDB!") + " " + Theme.muted(Theme.arrow() + " ");
            case IDLE:
            default:          return Theme.brand("miniDB") + " " + Theme.muted(Theme.arrow() + " ");
        }
    }
