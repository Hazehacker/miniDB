...
public class Server {
    ...
    public void start() {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(this::shutdown, "minidb-shutdown"));

        log.info("miniDB server listening on port {}", port);
        try {
            while (running.get()) {
                Socket socket;
                try {
    ...
    public void shutdown() {
        ...
        log.info("shutting down miniDB server...");
        ...
        log.info("miniDB server stopped.");
    }
}
