...
java -cp "target/classes;$(cat data/cp.txt)" top.tankenqi.zingdb.engine.Launcher -create data/db

# 起服务端（阻塞在 9999 端口）
java -cp "target/classes;$(cat data/cp.txt)" top.tankenqi.zingdb.engine.Launcher -open data/db

# 另开一个终端：客户端
java -cp "target/classes;$(cat data/cp.txt)" top.tankenqi.zingdb.cli.Launcher
```
...
begin; insert into users values (3, 'carol', 41); abort;   -- 回滚关键字是 abort
show tables;
```
...
```bash
java -cp "target/classes;$(cat data/cp.txt)" top.tankenqi.zingdb.cli.Launcher --no-color -e "select * from users"
java -cp "target/classes;$(cat data/cp.txt)" top.tankenqi.zingdb.cli.Launcher -f schema.sql
```
...
- **中文乱码**：需要 `chcp 65001` + `-Dstdout.encoding=UTF-8`。非 UTF 控制台可临时 `ZINGDB_UNICODE=0` 退回 ASCII 表格；`ZINGDB_COLOR=0` 或 `--no-color` 关颜色。
- **6 个测试类硬编码了 `/tmp/zingdb`**（`BPlusTreeTest`、`DataManagerTest`、`ExecutorTest`、`LoggerTest`、`PageCacheTest`、`TransactionManagerTest`）。Windows 上 `C:\tmp` 不存在 → `Panic.panic` 调 `System.exit(1)` → **surefire fork 直接终止**，报 "VM terminated without properly saying goodbye"。排除这 6 个后 `mvn test` 正常。修法是换成 `System.getProperty("java.io.tmpdir")`。
